...
```{r}
dat[ , .(mean_critical = mean(num_critical), 
...
ggplot(dat, aes(num_critical)) + geom_histogram(binwidth = 1) + 
  labs(title = "Histogram of Number of Critical Violations") 
ggplot(dat, aes(num_non_critical)) + geom_histogram(binwidth = 1) + 
...
dat[ , .(mean_num_critical = mean(num_critical)), 
  ...
  geom_point() + 
  geom_line() + 
  labs(title = "Mean Number of Critical Violations by Days Since Last Inspection
...
fit <- glm(num_critical ~ num_critical_previous + zip + City, 
...
dat <- subset(dat, select = c("num_critical_binary", "num_critical_previous_binary", 
                              "zip", "City"))
dat <- dat[complete.cases(dat)]  # make complete ONLY for quick modelling
fit <- glm(num_critical_binary ~ num_critical_previous_binary + zip + City, 
           data = dat, family = "binomial")
summary(fit)
fitted_values <- predict(fit, type = "response")
pred <- prediction(fitted_values, dat$num_critical_binary)
...
```{r previousAgree}
dat[ , mean(num_critical_binary == num_critical_previous_binary)]
```

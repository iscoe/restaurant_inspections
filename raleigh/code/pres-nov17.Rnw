...
Specifically, what is in the model are these variables:

\begin{itemize}
  \item Number of critical violations in previous inspection
  \item Days since first opening date of restaurant
  \item Average neighbors previous number of critical violations
  \item Median household income in the encompassing ZIP
  \item Percent of Families Below Poverty Line 
  \item Yelp rating, price, categories
\end{itemize}
